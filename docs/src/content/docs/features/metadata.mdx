...
However, the max age would not be working properly for events, which are going to exceed the age in the source cluster. That is because in the target cluster all the events will appear as recently added.
