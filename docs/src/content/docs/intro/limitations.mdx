...
description: Be aware of these Replicator limitations
sidebar:
  order: 3
---
...
Replicator will copy all the stream metadata. However, the max age set on a stream will not be set as expected, because all the events in the target cluster will be assigned a new date. The `$originalCreatedDate` metadata field might help to mitigate this issue.
